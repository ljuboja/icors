<?php
update_option('siteurl','http://www.ico.rs');
update_option('home','http://www.ico.rs');
...
function unspoken_setup() {
    ...
    if ( get_option('unspoken_mag_use') ) add_image_size( 'slide-magazine', 620, 380, true );
...
function unspoken_get_tags() {
    $tags = wp_tag_cloud('smallest=8&largest=20&number=25&orderby=name');
    if ( $tags ) {
        $output = __( '<p>Oznake</p>', 'unspoken' );
        $i = 0;
        foreach ($tags as $tag){
            $i++;
...
function portfolio_posts_per_page( $query ) {
    if ( $query->query_vars['post_type'] == 'radio_odzaci_blog' ) $query->query_vars['posts_per_page'] = 10;
    return $query;
}
...
/* If you want to unset certain WP SEO columns for all users:
 * uncomment the following line, load any admin page,
 * comment back (this only needs to run once).
 *
 * This is a weird attempt to work around having to edit
 * screen options for each user on a site that has more
 * than a few registered users (see the comment linked below:
 *
 * http://wordpress.org/support/topic/plugin-wordpress-seo-by-yoast-option-to-disableenable-columns-in-all-posts?replies=30#post-3384463
 */
//add_action( 'admin_init', 'global_unset_wp_seo_columns' );
...
} // end global_unset_wp_seo_columns
