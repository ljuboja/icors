<?php if ( comments_open() ) : global $post, $id, $comment, $user_login, $user_ID, $user_identity; ?>
    ...
    <div id="respond">
         <div class="block-title"><?php _e( 'Pošalji komentar', 'unspoken' ); ?></div>
                 ...
                 <form action="<?php bloginfo('url'); ?>/wp-comments-post.php" method="post" id="commentform" <?php if ( get_option('require_name_email') && !$user_ID  ) { echo 'onSubmit="return validate(false);"'; } else { echo 'onSubmit="return validate(true);"'; } ?>>
                     <fieldset>
                         ...
                         <?php if ( !$user_ID ) : ?>
                             ...
                             <tr>
                                 <td class="label"><label for="author"><?php _e( 'Vaše ime', 'unspoken' ); ?></label></td>
                                 <td>
                                     <p><input type="text" name="author" id="author" tabindex="1" class="input"/></p>
                                     <p class="alert"><?php _e('Molimo Vas upišite Vaše ime', 'unspoken'); ?></p>
                                 </td>
                                 <td><span><?php ( get_option('require_name_email') ) ? _e( 'Ime je obavezno', 'unspoken' ) : ''; ?></span></td>
                             </tr>
                             <tr>
                                 <td class="label"><label for="email"><?php _e( 'Vaš email', 'unspoken' ); ?></label></td>
                                 <td>
                                     <p><input type="text" name="email" id="email" tabindex="2" class="input"/></p>
                                     <p class="alert"><?php _e('Molimo Vas unesite ispravan email', 'unspoken'); ?></p>
                                 </td>
                                 <td><span><?php ( get_option('require_name_email') ) ? _e( 'Email je obavezan', 'unspoken' ) : ''; ?></span></td>
                             </tr>
                             <tr>
                                 <td class="label"><label for="url"><?php _e( 'Web sajt', 'unspoken' ); ?></label></td>
                                 <td>
                         ...
                         <?php endif; // if ( $user_ID ) ?>
                             ...
                             <tr>
                                 <td  class="label"><label for="comment"><?php _e( 'Tekst komentara', 'unspoken' ); ?></label></td>
                                 <td colspan="2">
                                     <p><textarea name="comment" id="comment" cols="30" rows="10" tabindex="4"></textarea></p>
                                     <p class="alert"><?php _e('Molimo Vas unesite tekst komentara', 'unspoken'); ?></p>
                             ...
                             <tr>
                                 ...
                                 <td colspan="2" class="submit">
                                     <div>
                                         <input id="submit" type="submit" value="<?php _e('Pošalji komentar', 'unspoken'); ?>" tabindex="5"/><span id="cancel-comment-reply"><?php cancel_comment_reply_link(__('Prekini', 'unspoken')); ?></span>
                                     </div>
                                 </td>
                             </tr>
                         </table>
